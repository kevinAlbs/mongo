...
public:
    DensityEstimator(PlanStage::Children* children,
        ...
        : _children(children), _s2Index(s2Index), _nearParams(nearParams), _indexParams(indexParams), _currentLevel(0) {
...
PlanStage::StageState GeoNear2DSphereStage::initialize(OperationContext* txn,
                                                       ...
                                                       WorkingSetID* out) {
    if (!_densityEstimator) {
        _densityEstimator.reset(new DensityEstimator(&_children, _s2Index, &_nearParams, _indexParams));
    }
