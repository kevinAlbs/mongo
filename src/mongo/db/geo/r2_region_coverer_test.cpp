...
namespace {
    ...
    std::vector<GeoHash> generateAdjacentCells(long long startingHash,
                                               unsigned bits,
                                               long long numCells) {
        std::vector<GeoHash> cellIds;
        for (long long i = 0; i < numCells; i++) {
            long long offset = i << (2 * (32 - bits));
    ...
    NumType random(NumType num) {
        std::uniform_int_distribution<NumType> distribution(0, num - 1);
...
>>>>>>> r2
    ...
    TEST(R2CellUnion, Intersects) {
        ...
        for (int k = 0; k < 2000; ++k) {
            ...
            long long disjointOffset = 1LL << (2 * (32 - bits));
    ...
    TEST(R2CellUnion, Difference) {
        for (int i = 0; i < 2000; ++i) {
            ...
            long long offset = (numCells/2) << (2 * (32 - bits));
            std::vector<GeoHash> yCellIds =
                    generateAdjacentCells(startingHash + offset, bits, numCells);
